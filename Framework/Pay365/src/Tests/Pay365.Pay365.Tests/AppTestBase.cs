...
using Pay365.Pay365.Migrations.Seed.Host;
using Pay365.Pay365.Migrations.Seed.Tenants;
using Pay365.Pay365.MultiTenancy;
using Pay365.Pay365.Tests.TestDatas;
...
namespace Pay365.Pay365.Tests
{
    ...
    /// Provides methods to easily work with <see cref="Pay365DbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<Pay365TestModule>
    {
        ...
        protected void UsingDbContext(Action<Pay365DbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Pay365DbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Pay365DbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Pay365DbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Pay365DbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Pay365DbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Pay365DbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Pay365DbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Pay365DbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Pay365DbContext, Task<T>> func)
        {
            T result;
