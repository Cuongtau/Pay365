        ...
        utils.postData(api,
            { Otp: otp }, function (data) {
                ...
                if (data.c >= 0) {
                    //do sth
                    if (header.AccountInfo.CurrentLang === 'en') {
                        ModalNotificationResultInit(null, (action !== 'offline' ? null : 'Your withdrawal requests has been sent. We will inform you in 24h of working time'), utils.renderModalContent({ _transid: data.d.TransactionID, _totalamount: (!data.d.Amount ? '' : (utils.formatMoney(data.d.Amount) + 'VNĐ')), _balance: ((!data.p || data.p.length === 0) ? '' : (utils.formatMoney(data.p[0]) + 'VNĐ')) }, 'cashout'), 'Go home page', 'Continue cashout', function () { window.location.href = utils.rootUrl() + 'thong-tin'; }, function () { window.location.href = utils.rootUrl() + 'rut-tien'; });
                    } else {
                        ModalNotificationResultInit(null, (action !== 'offline' ? null : 'Gửi yêu cầu rút tiền thành công. Hệ thống sẽ xử lý và thông báo trong vòng 24h làm việc'), utils.renderModalContent({ _transid: data.d.TransactionID, _totalamount: (!data.d.Amount ? '' : (utils.formatMoney(data.d.Amount) + 'VNĐ')), _balance: ((!data.p || data.p.length === 0) ? '' : (utils.formatMoney(data.p[0]) + 'VNĐ')) }, 'cashout'), 'Về trang chủ', 'Tiếp tục rút', function () { window.location.href = utils.rootUrl() + 'thong-tin'; }, function () { window.location.href = utils.rootUrl() + 'rut-tien'; });
                    }
