...
account_info = new function () {
    ...
    this.Vw_ChangeAccountInfo = function () {
        ...
        setTimeout(function () {
            ...
            if (header.AccountInfo.National.length > 0) {
                $.each(header.AccountInfo.National, function (key, val) {
                    var script;
                    if (nation == 0) {
                        if (val.NationalID == 84) {
                            script = "<option selected='selected' value='" + val.NationalID + "'>" + val.NationalName + "</option>";
                        } else
            ...
            if (header.AccountInfo.Location.length > 0) {
                $.each(header.AccountInfo.Location, function (key, val) {
                    var script;
            ...
            if (header.AccountInfo.District.length > 0) {
                $.each(header.AccountInfo.District, function (key, val) {
                    var script;
                    if (val.DistrictID == header.AccountInfo.DistrictID)
                        script = "<option selected='selected' value='" + val.DistrictID + "'>" + val.DistrictName + "</option>";
                    else
    ...
    this.get_National = function (nation) {
        ...
        utils.postData(utils.profileApi() + 'profile/GetNational', params, function (data) {
    ...
    this.get_Location = function (nation, location) {
        ...
        utils.postData(utils.profileApi() + 'profile/GetLocation', params, function (data) {
    ...
    this.get_District = function (location, district) {
        ...
        utils.postData(utils.profileApi() + 'profile/GetDistrict', params, function (data) {
    ...
    this.onchange_Nation = function (t) {
        ...
        utils.postData(utils.profileApi() + 'profile/GetLocation', params, function (data) {
            if (data.length > 0) {
                ...
                $("#ddllocation").append("<option value='0'>" + utils.getCurrentLanguage() === 'en' ? 'No Data' : 'Không có dữ liệu' + "</option>");
                $('select').material_select();
            }
        }, function (err) {
            console.log(err);
        ...
        utils.postData(utils.profileApi() + 'profile/GetDistrict',
            {
                ...
                if (data.length > 0) {
                    ...
                    $("#ddldistrict").html("");
                    $("#ddldistrict").append("<option value='0'>" + utils.getCurrentLanguage() === 'en' ? 'No Data' : 'Không có dữ liệu' + "</option>");
                    $('select').material_select();
                }
            }, function (err) {
                console.log(err);
        ...
        if (!$.isEmptyObject(params)) {
            ...
            var callbackFail = function (data) {
                ...
                if (utils.checkResponseIsValid(data)) {
                    ...
                    ModalNotificationInit(common.getDescription(responseCode), null, 'danger', null, header.AccountInfo.CurrentLang === 'en' ? 'Close' : 'Đóng', true);
                }
                else {
                    ModalNotificationInit(common.getDescription(-999999), null, 'danger', null, header.AccountInfo.CurrentLang === 'en' ? 'Close' : 'Đóng', true);
                }
            ...
            ModalNotificationInit(utils.getCurrentLanguage() === 'en' ? 'Please select a certification photo' : 'Vui lòng chọn ảnh chứng thực');
        }
    };
    ...
    this.VerifyEmailView = function (isResend) {
        ...
        if (isResend && $('#resend_verify_code').data('resend_verify_code')) {
            $('#modal-alert #modal_content').text(utils.getCurrentLanguage() === 'en' ? 'Time between 2 re-send secure code is minimum 60s' : 'Khoảng cách giữa 2 lần gửi lại mã xác thực tối thiểu là 30s');
            $('#modal-alert').modal('open');
            return;
        }
        ...
        var callback = function (data) {
            ...
            if (data.c >= 0) {
                if (!isResend) {
                    ...
                    $('#modal-alert #modal_content').text(utils.getCurrentLanguage() === 'en' ? 'Resend verify email code successfully' : 'Nhận lại mã xác thực thành công');
                    $('#modal-alert').modal('open');
    ...
    this.ConfirmVerifyEmail = function () {
        ...
        var callback = function (data) {
            ...
            if (data.c >= 0) {
                ...
                $('#modal-success #modal_content').text(utils.getCurrentLanguage() === 'en' ? 'Verify email successfully' : 'Xác thực email thành công');
                $('#modal-success').modal('open');
                ...
                if (data.c === 1) {
                    if (header.AccountInfo.CurrentLang === 'en') {
                        if (header.AccountInfo.SecurityType === common.accountSecureConfig.EMAIL) {
                            $('<p>An OTP has been sent to email <span class="secondary">' + header.AccountInfo.Email + '</span>.</br> Do not receive OTP, please click <span class="secondary">Resend OTP</span></p>')
                        ...
                        if (header.AccountInfo.SecurityType === common.accountSecureConfig.EMAIL) {
                            $('<p>Mã xác thực OTP đã được gửi đến email <span class="secondary">' + header.AccountInfo.Email + '</span>.</br> Không nhận được mã, vui lòng click <span class="secondary">Nhận lại OTP</span></p>')
                                .insertAfter($('#formChangeEmail #div_otp_change_email .form-group'));
                            $('#formChangeEmail #change_email_resend_otp').show();
                        } else if (header.AccountInfo.SecurityType === common.accountSecureConfig.SMS || header.AccountInfo.SecurityType === 0) {
    ...
    this.ConfirmChangeEmail = function () {
        ...
        var callback = function (data) {
            ...
            if (data.c >= 0) {
                ...
                $('#HiddenEmailVerify').parent('li').find('.data').html(newEmail + '<i class="material-icons danger">clear</i>');
                $('#mission_t div:first-child').html('<i class="material-icons danger">clear</i><p class="label">Xác thực <strong>Email</strong></p><p class="text"><a href="javascript:;" onclick="account_info.VerifyEmailView();" class="btn btn-sm btn-primary waves-effect waves-light">Xác thực email</a></p>');
                ...
                ModalNotificationInit(utils.getCurrentLanguage() === 'en' ? 'Change email successfully' : 'Thay đổi email thành công', closeCallBack);
        ...
        utils.loading();
        utils.postData(utils.linkIdApi() + 'Account/VerifyEmailChange', { EmailNew: newEmail, Otp: otp }, callback, callbackFail);
    }
...
transaction_history = new function () {
    ...
    this.GetServices = function () {
        utils.getData(utils.trasactionApi() + "Payment/GetServices", { ServiceType: 0 }, function (data) {
    ...
    this.GetListHistory = function () {
        ...
        utils.postData(utils.trasactionApi() + "Payment/GetHistory", params, function (data) {
            if (data.p != null && data.p.length > 0) {
                ...
                common.saveLog(data);
                $('#content_data, #export_excel').hide(); $('#content_empty').show();
                $('#divpager').html('');
                return;
            }
            var totalPage = transaction_history.listHistory.listSuccess.length > transaction_history.listHistory.listPending.length ? Math.ceil(transaction_history.listHistory.listSuccess.length / transaction_history.listHistory.itemPerPage) : Math.ceil(transaction_history.listHistory.listPending / transaction_history.listHistory.itemPerPage);
            transaction_history.listHistory.totalPage = totalPage;
            //Intit Pager
            var $pageID = $('#divpager');
            $pageID.html('');
            //append left icon
            $pageID.append('<a href="javascript:;" id="previous" class="btn btn-page waves-effect waves-primary"><i class="material-icons">chevron_left</i></a>');
            //append page-item

            for (var i = 0; i < totalPage; i++) {
                if (i === 0) //Page 1 actived
                    $pageID.append('<a href="javascript:;" class="btn btn-page waves-effect waves-primary active page-item">' + (i + 1) + '</a>');
                else
                    $pageID.append('<a href="javascript:;" class="btn btn-page waves-effect waves-primary page-item">' + (i + 1) + '</a>');
            }
            //append right icon
            $pageID.append('<a href="javascript:;" id="next" class="btn btn-page waves-effect waves-primary"><i class="material-icons">chevron_right</i></a>');

            $pageID.find('.page-item:gt(' + transaction_history.listHistory.paginationShow + ')').hide();

            //Load page 1
            var listSuccessInitPage = transaction_history.listHistory.listSuccess.slice(0, transaction_history.listHistory.itemPerPage);
            var listPendingInitPage = transaction_history.listHistory.listPending.slice(0, transaction_history.listHistory.itemPerPage);
    ...
    this.LoadPage = function (page) {
        var listSuccess = transaction_history.listHistory.listSuccess.slice((page - 1) * transaction_history.listHistory.itemPerPage, page * transaction_history.listHistory.itemPerPage);
        var listPending = transaction_history.listHistory.listPending.slice((page - 1) * transaction_history.listHistory.itemPerPage, page * transaction_history.listHistory.itemPerPage);
        ...
        if (page === lastPageVisible) {
            ...
            $pageID.find('.page-item:gt(' + (lastPageVisible + transaction_history.listHistory.paginationShow - 2) + ')').hide();
            ...
            $pageID.find('.page-item:lt(' + (firstPageVisible - transaction_history.listHistory.paginationShow) + ')').hide();
    ...
    this.ShowCardInfo = function (step, t) {
                ...
                if (data.c >= 0) {
                    ...
                    if (data.d && data.d.length > 0) {
                        ...
                        $.each(data.d, function (key, val) {
                            $tbody.append('<tr><td align="center"><input type="checkbox" value="' + val.ProductItemID + '" class="filled-in checkbox_i" id="i_' + val.ProductItemID + '"><label for="i_' + val.ProductItemID + '"></label></td><td align="center">' + val.Number + '</td><td align="center">' + val.ProductName + '</td><td align="center">' + utils.formatMoney(val.Value) + '<sup>VNĐ</sup></td><td align="center">' + val.Code + '</td><td align="center">' + val.Serial + '</td><td align="center">' + utils.formDateTime(val.ExpriredDate) + '</td></tr>');
                        });
        ...
        if (!otp) {
            ...
            $inputOTP.parent().find('.error-text').text(header.AccountInfo.CurrentLang === 'en' ? 'Please enter secure code' : 'Vui lòng nhập mã bảo mật');
            $inputOTP.focus();
            return;
        }
        var orderID = $(t).data('orderid');
        utils.postData(utils.trasactionApi() + "Payment/GetCardValue", { OrderID: orderID, Otp: otp }, function (data) {
            if (data.c >= 0) {
                $formDetail.find('#table_card').show();
                $formDetail.find('#show_step2').hide();
                if (data.d && data.d.length > 0) {
    ...
    this.ShowCardBetween = function (t) {
        ...
        if (from && to) {
            to = parseInt(to);
            from = parseInt(from);
            if (from > to) { transaction_history.listCard.dataSelected = []; return; }
    ...
    this.ExportExcel = function () {
        ...
        jsExportExcel.download(listExport, 'lsgd_' + new Date().getTime() + '.xls', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'export_excel');
    };
    ...
    this.ResendOTPEmail = function (t) {
        if ($(t).data('resend_verify_code')) {
            ModalNotificationInit(header.AccountInfo.CurrentLang === 'en' ? 'Time between 2 re-send secure code is minimum 30s' : 'Khoảng cách giữa 2 lần gửi lại mã xác thực tối thiểu là 30s');
            return;
        }
        utils.postData(utils.trasactionApi() + "Payment/ResendOTP",
            ...
            function (data) {
                utils.unLoading();
                if (data.c >= 0) {
                    ModalNotificationInit(utils.getCurrentLanguage() === 'en' ? 'Resend secure code successfully' : 'Nhận lại mã bảo mật thành công');
    ...
    this.Resend_OTP = function () {
        ...
        utils.loading();
        utils.postData(urlPaymentApi, {}, function () {
            utils.unLoading();
    ...
    this.get_History_Home = function () {
        ...
        utils.postData(utils.trasactionApi() + "Payment/GetHistory", { Status: 1, Top: 2 }, function (data) {
            if (data.p.length > 0) {
                console.log(data.p);
        ...
        utils.postData(utils.trasactionApi() + "Payment/GetHistory", { Status: 0, Top: 2 }, function (data) {
            if (data.p.length > 0) {
                console.log(data.p);
