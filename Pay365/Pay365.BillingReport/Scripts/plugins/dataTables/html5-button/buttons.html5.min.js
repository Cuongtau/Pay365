(function (g) {
        ...
        action: function (a, b, e, c) {
            ...
            e = function (a, tag, index, lst_row) {
                ...
                for (cellIdx = 0; cellIdx < a.length; cellIdx++) {
                    ...
                    var re = new RegExp(',', 'g');
                    var val = a[cellIdx].toString();
                    var numb = val.replace(re, "");
                    if (numb.indexOf('T') > 0 && numb.indexOf('.') > 0) {
                        var pos = numb.indexOf('.');
                        numb = numb.substring(0, pos);
                    }
                    var checkNum = Utils.ReplaceAll(numb, '.', '');
                    //nếu ko phải kiểu số

                    if (!$.isNumeric(checkNum)) {
                        debugger
                        if (Utils.validateDateTime(checkNum)) {
                            if (index == 0) {
                    ...
                    if (countRowspan == 0) {
                        ...
                        var data = (!a[cellIdx].replace ? a[cellIdx] : a[cellIdx].replace(/&(?!amp;)/g, "&amp;").replace(/[\x00-\x1F\x7F-\x9F]/g, ""));

                        if ($.isNumeric(checkNum) || a[cellIdx].match && a[cellIdx].match(/^-?[0-9\.\,]+$/)) {
                            if (checkNum.indexOf("0") == 0 || $.type(a[cellIdx]) == "string") {
                                b.push('<' + tag + ' rowspan="' + countRowspan + '"  align="center" style="border:1px solid #848484; vertical-align: middle;text-align:center" class="text">' + " " + a[cellIdx] + '</' + tag + '>');
                            }
                            else {
                                b.push('<' + tag + ' rowspan="' + countRowspan + '"  align="right" style="border:1px solid #848484; vertical-align: middle;text-align:right">' + checkNum + '</' + tag + '>');
                            }
                        }
                        else {
                            var pos = data.indexOf('.');
                            if (pos >= 0) {
                                if ((data.match(new RegExp(".", "g"))).length == 1) {
                                    data = data.substring(0, pos);
                                }
                            }
                            //var date = new Date(data);
                            if (Utils.validateDateTime(data)) {
                                data = data.replace("T", " ");// replace time
                                b.push('<' + tag + ' rowspan="' + countRowspan + '"  align="center" style="border:1px solid #848484; vertical-align: middle;">' + Utils.dateTypeFormatter(data, c.typeTime) + '</' + tag + '>');
                            }
                            else {
                                b.push('<' + tag + ' rowspan="' + countRowspan + '"  align="center" style="border:1px solid #848484; vertical-align: middle;">' + data + '</' + tag + '>');
                            }
                        }
                    }
                    else {
                        ...
                        if ($.isNumeric(checkNum) || a[cellIdx].match && a[cellIdx].match(/^-?[0-9\.\,]+$/)) {
                            if (checkNum.indexOf("0") == 0 || $.type(a[cellIdx]) == "string") {
                                b.push('<' + tag + ' align="center" style="border:1px solid #848484; vertical-align: middle;text-align:center" class="text">' + " " + a[cellIdx] + '</' + tag + '>');
                            }
                            else {
                                b.push('<' + tag + ' align="right" style="border:1px solid #848484; vertical-align: middle;text-align:right">' + checkNum + '</' + tag + '>');
                            }
                        }
                        else {
                            var pos = data.indexOf('.');
                            ...
                            if (Utils.validateDateTime(data)) {
                                data = data.replace("T", " ");// replace time
                                b.push('<' + tag + ' align="center" style="border:1px solid #848484; vertical-align: middle;">' + Utils.dateTypeFormatter(data, c.typeTime) + '</' + tag + '>');
                            }
                            else {
                                b.push('<' + tag + ' align="center" style="border:1px solid #848484; vertical-align: middle;">' + data + '</' + tag + '>');
                            }
                        }
                    }
                }
    ...
    || /1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(navigator.userAgent.substr(0, 4))) {
            ...
            m["[Content_Types].xml"]); e.file(".rels", m["_rels/.rels"]); f.file("workbook.xml", m["xl/workbook.xml"]); h.file("workbook.xml.rels", m["xl/_rels/workbook.xml.rels"]); g.file("sheet1.xml", m["xl/worksheets/sheet1.xml"].replace("__DATA__", a)); j(b.generate({ type: "blob" }), t(c))
                ...
                docData += '<table width="100%" cellspacing="0" border="1"><thead>';
                // Header
                var $hrows = [];
                var $rows = [];
                ...
                if (MSDocType === 'excel') {
                    ...
                    docFile += "</xml>";
                    docFile += "<![endif]-->";
                    docFile += '<xsl:template name="styles">' +
                ...
                b.push(
                    ...
                    g.map(bodyObj[f], function (a) {
                        ...
                        if (!Number.isInteger(a)) {
                            if (a.toString().indexOf('T') > 0 && a.toString().indexOf('.') > 0) {
                                var pos = a.toString().indexOf('.');
                                numb = a.toString().substring(0, pos);
                            }
                            var checkNum = Utils.ReplaceAll(numb, '.', '');
                            //numb = a.replace(re, '');
                            //numb = Utils.ReplaceAll(a, '.', '');
                            if (Utils.validateDateTime(checkNum)) { // kiểu time
                                a = a.replace("T", " ");  //replace time
                                numb = Utils.dateTypeFormatter(a, 1)
                            } else {
                                numb = a;
                            }
                        } else {
                        ...
                        if (!$.isNumeric(numb)) {
                            ...
                            if (a.toString().indexOf("0") == 0) { // là số Điện Thoại 
                                numb = a;
                            } else {
                                if (Number.isInteger(numb)) {
                                    numb = Utils.formatMoney(a.toString());
                                } else {
                                    numb = a;
                                }
                            }
                            obj.text = numb.toString();
                            cell++;
                        }
                        return obj;
                    })
