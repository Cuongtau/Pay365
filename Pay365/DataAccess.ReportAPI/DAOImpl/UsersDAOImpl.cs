    ...
    public class UsersDAOImpl : IUsersDAO
    {
        ...
        public int Authentication(string username, string password)
        {
            try
            {
                ...
                new DBHelper(Config.AdminReportConnectionString).ExecuteNonQuerySP("sp_User_Authenticate", pars);
        ...
        public Users SelectByUserID(int userId)
        {
            try
            {
                ...
                return new DBHelper(Config.AdminReportConnectionString).GetInstanceSP<Users>("SP_User_GetByCondition",
        ...
        public List<Users> GetListUsers(string Keyword, int isActive, int isGrant)
        {
            try
            {
                ...
                var list = new DBHelper(Config.AdminReportConnectionString).GetListSP<Users>("SP_User_GetPage", pars);
        ...
        public List<Users> GetAllUserByStatus(int Status)
        {
            try
            {
                var list = new DBHelper(Config.AdminReportConnectionString)
                    .GetListSP<Users>("SP_User_GetByCondition", new SqlParameter("@_Status", Status));
                if (list == null || list.Count <= 0)
                    return new List<Users>();
                return list;
            }
        ...
        public int UpdateUsers(Users users)
        {
            try
            {
                ...
                new DBHelper(Config.AdminReportConnectionString).ExecuteNonQuerySP("SP_User_InsertUpdate", pars);
        ...
        public int DelleteUsers(int userId)
        {
            try
            {
                ...
                new DBHelper(Config.AdminReportConnectionString).ExecuteNonQuerySP("SP_User_Delete", pars);
        ...
        public int UpdateActiveUser(int Id)
        {
            try
            {
                ...
                pars[1] = new SqlParameter("@_ResponseCode", SqlDbType.Int) { Direction = ParameterDirection.Output };
                new DBHelper(Config.AdminReportConnectionString).ExecuteNonQuerySP("SP_User_UpdateActive", pars);
                return Convert.ToInt32(pars[1].Value);
            }
            catch (Exception e)
            {
        ...
        public int ChangePassword(string UserName, string PasswordOld, string PasswordNew)
        {
            try
            {
                ...
                new DBHelper(Config.AdminReportConnectionString).ExecuteNonQuerySP("SP_User_ChangePassword", pars);
                return Convert.ToInt32(pars[3].Value);
            }
            catch (Exception e)
            {
        ...
        public int UpdateUserLock(int Id, bool isLock)
        {
            try
            {
                ...
                new DBHelper(Config.AdminReportConnectionString).ExecuteNonQuerySP("SP_User_Update_Lock", pars);
